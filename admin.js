/* =====================================================================
   ...
   atualização enviados pelo público.
   ===================================================================== */
(function () {
  "use strict";
  ...
  function inst(id){ return (catalogo.instituicoes||[]).find(function(i){return i.id===id;}); }
  ...
  async function enviarImagem(id,file){
    ...
    try{
      ...
      toast("Foto atualizada. (lembre que ela já foi salva)");
  ...
  async function aplicar(reqId){
    ...
    var o=inst(p.instituicaoId);
    if(!o){ toast("A instituição deste pedido não existe mais.",true); return resolver(reqId,"rejeitado"); }
    Object.keys(p.campos||{}).forEach(function(k){ o[k]=p.campos[k]; });
    try{
      await Store.putCatalogo(catalogo);     // publica no catálogo
      await resolver(reqId,"aprovado");      // remove da caixa
      atualizarMedidor(); renderAbas(); render();
  ...
  async function rejeitar(reqId){
    ...
    try{ await resolver(reqId,"rejeitado"); renderAbas(); render(); toast("Pedido rejeitado."); }
    catch(e){ toast("Falha ao atualizar a caixa.",true); }
  }
  /* remove o pedido da caixa de entrada (mantém a caixa enxuta) */
  async function resolver(reqId){
    caixa.pedidos=(caixa.pedidos||[]).filter(function(x){return x.id!==reqId;});
    await Store.putCaixa(caixa);
  }
